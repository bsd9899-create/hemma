...
-- ============================================================
-- شغّله دفعةً واحدة في:
--   Supabase Dashboard → SQL Editor → New query → لصق الملف كاملًا → Run
--
-- يحتوي الترحيلات السبعة التي لم تُطبَّق بعد على المشروع الحيّ:
...
--    DROP IF EXISTS، أو داخل حارس DO يفحص pg_type/pg_constraint أولًا،
--    أو GRANT/COMMENT (تكرارهما بلا أثر). لا توجد في الملف جملة
--    create type أو create table أو add column واحدة بلا حارس —
--    واختبار آلي (supabase/__tests__/migrationSafety.test.ts) يفشل
--    إن تسللت واحدة مستقبلًا.
--
--    مُثبت بالتشغيل الفعلي على PostgreSQL 16.13، لا بالقراءة:
--      • على قاعدة تحمل الترحيلات 1..10 فقط  → ثلاث تشغيلات متتالية، صفر أخطاء
--      • على قاعدة تحمل الترحيلات السبعة عشر → تشغيلان، صفر أخطاء
--      • على قاعدة تحمل بيانات مستخدم حقيقية → البيانات و٣٥ سياسة RLS كما هي
--
...
--
-- الملف كله داخل معاملة واحدة (begin/commit): إن فشلت أي جملة لأي
-- سبب، تُلغى كل التغييرات ولا تبقى القاعدة في حالة نصفية.
--
...
-- ============================================================
-- 20260906000005_food_analysis_usage.sql
-- ============================================================
-- ============================================================
